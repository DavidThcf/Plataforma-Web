...
module.exports.updateCategories = function (data) {
    var id_categoria = data.id_categoria;
    var nombre = data.nombre;    
    var color = data.color;

    var query1 = `
    update categorias_mapa c set nombre ='`+ nombre +`' ,color = '`+ color +`' where c.id_categoria = `+ id_categoria +` ;
    `;
    console.log('\n\n\n\nQuery1 ===>  ' + query1);
    return new Promise((resolve, reject) => {
        var sequelize = sqlCon.configConnection();
        ...
        .then(x => {
            ...
            `;
            fs.writeFile('files/Category/' + id_categoria + '.svg', img, function (err) {
                if (err) console.log('\n\n\nHubo un error' + err);
                console.log('Se creo exitosamente el icono de la categoria');
            });
            resolve(true);
...
module.exports.getCategoriesList = function (data) {
    ...
    if(opt==1){
        ...
        query1 =  `
        SELECT c.id_categoria,c.nombre,c.color ,count(m.id_categoria) as cont  FROM categorias_mapa c left join marcador m on c.id_categoria = m.id_categoria  
        where c.keym_car = `+ keym_car + ` 
        and c.id_caracteristica = `+ id_caracteristica + `   
        and c.id_usuario_car =   `+ id_usuario_car + ` 
        group by c.id_categoria order by c.id_categoria
        `

        query1 = `
        select * from getmarkeractivity(`+ keym_car + `,`+ id_caracteristica + `,`+ id_usuario_car + `);
        `
    }
