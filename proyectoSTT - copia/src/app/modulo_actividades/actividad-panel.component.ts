...
export class ActividadPanel implements OnInit{	
	...
	downloadCanvas(event){
		...
		this.imgReport = document.getElementsByTagName('canvas')[0].toDataURL('image/png')
		//anchor.download = "imagen.png";
		var formData = new FormData()
		formData.append("imgReport",this.imgReport)
		this.servicios.report(formData).
		then(pdf => {
			alert(pdf)
			anchor.open =
			anchor.download = pdf;
